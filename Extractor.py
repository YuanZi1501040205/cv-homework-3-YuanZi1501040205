...
path_output = '/project/kakadiaris/yz/dataset/images/'
path_output_train = path_output + 'train/'
path_output_test = path_output + 'test/'
path_data1 = '/project/kakadiaris/yz/dataset/parking1a/'
path_data2 = '/project/kakadiaris/yz/dataset/parking1b/'
path_data3 = '/project/kakadiaris/yz/dataset/parking2/'
path_data = []
    ...
    for folders in folders_under_root:
        ...
        for path in paths_image:
            ...
            for space in spaces:
                ...
                if count_neg <10000 and occupied == '0':
                    center = tuple(
                    ...
                    if roi.shape[0] !=0 and roi.shape[1] != 0 and roi.shape[2] !=0:
                        ...
                        cv2.imwrite('./images/train/neg/' + root_path.split("/")[-2] + '_' + path.split("/")[-1].split("\\")[-1].split(".")[0] + '_no' + id + '_slots.jpg', roi)
                        ...
                        pass
                elif count_pos < 14000 and occupied == '1':
                    rotatedRect = space.find('rotatedRect')
                    contour = space.find('contour')
                    id = space.attrib['id']
                    if len(space.attrib) == 1:
                        ...
                        if roi.shape[0] != 0 and roi.shape[1] != 0 and roi.shape[2] != 0:
                            ...
                            # save gray scale resized roi images to pos folder
                            cv2.imwrite('./images/train/pos/' + root_path.split("/")[-2] + '_' +
                                        path.split("/")[-1].split("\\")[-1].split(".")[
                                            0] + '_no' + id + '_slots.jpg', roi)
                            train_annotation_pos.append([root_path.split("/")[-2] + '_' +
                                                         path.split("/")[-1].split("\\")[-1].split(".")[
                                                             0] + '_no' + id + '_slots.jpg', '1', '0', '0', '45',
                                                         '50'])
...
print('Done!!!')
                    ...
                    # image_name = ET.SubElement(sample, 'image_name')
                    # label = ET.SubElement(sample, 'label')
                    # image_name.text = root_path.split("/")[-2] + '_'+ path.split("/")[-1].split("\\")[-1].split(".")[0] + '_no' + id +'_slots.jpg'
                    # label.text = occupied
                    #
                    # # create a new XML file with the results
                    # mydata = ET.tostring(sample)
                    # myfile = open('./xml/'+ root_path.split("/")[-2]+ '_' + path.split("/")[-1].split("\\")[-1].split(".")[0] + '_no' + id +'_slots.xml', "wb")
                    # myfile.write(mydata)
